import { pgTable, text, timestamp, bigint, jsonb, uuid, primaryKey, integer, check } from "drizzle-orm/pg-core";
...
export const products = pgTable("products", {
    ...
    icon : text().notNull(),
    url : text().notNull(),
    stats : jsonb().notNull().default({views : 0, visitors : 0}),
    profile_id : uuid().references(() => profiles.profile_id, {onDelete: "set null"}).notNull(),
    category_id : bigint({ mode: "number" }).references(() => categories.category_id, {onDelete: "cascade"}).notNull(),
